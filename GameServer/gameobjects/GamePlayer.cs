...
break;
        ...
        public virtual bool MoveToHouseBind()
        {
            Region rgn = WorldMgr.GetRegion((ushort)BindHouseRegion);
            if (rgn == null || rgn.GetZone(BindHouseXpos, BindHouseYpos) == null)
            {
                if (log.IsErrorEnabled)
                {
                    log.Error("Player: " + Name + " unknown bind point : (R/X/Y) " + BindHouseRegion + "/" + BindHouseXpos + "/" + BindHouseYpos);
                }
            ...
            if (GameServer.ServerRules.IsAllowedToMoveToBind(this))
            {
                return MoveTo((ushort)BindHouseRegion, BindHouseXpos, BindHouseYpos, BindHouseZpos, (ushort)BindHouseHeading);
            }

            return false;
        }

        public virtual bool MoveToGuildHouse()
        {
            House house = HouseMgr.GetGuildHouseByPlayer(Client.Player);
            ...
            else
            {
                ...
                OnDestinationPicked(Client.Player, teleport);
                return true;
            }
        }

...
//                                  "[The Inconnu Crypt] in the Catacombs\n" +
                    ...
                    else
                    {
                        ...
                        SayTo(player, "In addition to the other locations to which you may travel, you are eligible to teleport to the [Dartmoor], the Svartalf village in Malmohus.");
                    }
                    break;
...
*/
						...
						case "pvp":
                            if (!t.InCombat)
                            {
                                Say("I'm now teleporting you to the PvP Hub");
                                foreach (GamePlayer player in this.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
                                ...
                                if (!t.InCombat)
                {
                    ...
                    player.Out.SendSpellCastAnimation(this, 4953, 6);
						...
						if (t.Realm == eRealm.Midgard && t.Level > 4 && t.Level < 40)
                        {
                        // Move to Molvik area 241
                        t.MoveTo(241, 549468, 577418, 5992, 2552);
                        }
						
						if (t.Realm == eRealm.Hibernia && t.Level > 4 && t.Level < 40)
                        {
                        // Move to Molvik area 241
                        ...
                        case "dartmoor":
                            if (t.Level < 100) // and a && check for the players flag for this, must complete a quest.
                            {
                                if (!t.InCombat)
                                {
                                    ...
                                    foreach (GamePlayer player in this.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
                                        player.Out.SendSpellCastAnimation(this, 4953, 3);
                                    t.MoveTo(1, 400420, 710381, 336, 254);
                                }
                                else { t.Client.Out.SendMessage("You can't port while in combat.", eChatType.CT_Say, eChatLoc.CL_PopupWindow); }
                            }
                            break;
...
*/
            ...
            {
                case "personal":
					if (t.BindHouseXpos !=0)
					{
						SayTo(t, "I shall take you to your house.");
						t.MoveToHouseBind();
					}
					if (t.BindHouseXpos == 0)
					{
						SayTo(t, "You don't have a house set.");
					}
                    break;
                case "guild":
                    if (t.Guild != null && t.Guild.GuildOwnsHouse == true)
					{
						SayTo(t, "I shall take you to your guild's house.");
						t.MoveToGuildHouse();
					}
					else { SayTo(t, "You must be in a guild that owns a guild house."); }
                    break;
                case "hearth":
